...
import { FlavorProfile, FlavorArchetype } from "@/lib/data";

interface FlavorRadarProps {
  data: FlavorProfile | FlavorArchetype;
}

export default function FlavorRadar({ data }: FlavorRadarProps) {
  const lastEntry = "complexity" in data
    ? { subject: "Complexity", A: data.complexity }
    : { subject: "Roasted",    A: (data as FlavorProfile).roasted };
